...
import { PREVIEW_READ_ONLY_MESSAGE, previewWriteGuard } from "../lib/userPreview";

export function AccountPage() {
  const { viewer, isDemoMode, isUserPreview, signOut, setLastAction } = useLeague();
  ...
  async function changePassword(event) {
    ...
    if (isUserPreview || previewWriteGuard.isLocked()) { setFeedback({ error: PREVIEW_READ_ONLY_MESSAGE }); return; }
  ...
  return <>
    ...
    <section className="panel account-settings">
      ...
      {isDemoMode ? <Notice tone="info">Las contraseñas solo se gestionan con una cuenta real. Esta demostración no modifica ninguna cuenta.</Notice> : <form className="stack-form" onSubmit={changePassword} aria-busy={busy}>
        <label>Contraseña actual<input type="password" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} required disabled={busy || isUserPreview} /></label>
        <label>Nueva contraseña<input type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={12} maxLength={128} required disabled={busy || isUserPreview} aria-describedby="password-help" /></label>
        <small id="password-help">Entre 12 y 128 caracteres. Puedes utilizar una frase que recuerdes.</small>
        <label>Repite la nueva contraseña<input type="password" autoComplete="new-password" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} minLength={12} maxLength={128} required disabled={busy || isUserPreview} /></label>
        ...
        <button className="button button-primary" disabled={busy || isUserPreview} type="submit">{busy ? "Guardando…" : "Guardar contraseña"}</button>
      </form>}
