...
export function ClubPortalPage() {
  ...
  if (!viewer) {
    return <section className="protected-page"><PageHero eyebrow="Área privada" title="Portal de clubes" description="Cada presidente podrá consultar su plantilla, preparar la formación 4+1 y seguir el estado de sus jornadas." /><Notice tone="warning">Inicia sesión desde “Acceso clubes” para abrir el portal de tu equipo.</Notice></section>;
  }
  if (!club) {
  ...
  const standing = standings.find((row) => row.club.id === club.id);
  ...
  return (
    <>
      <PageHero eyebrow={viewer.role === "admin" ? "Administrador + presidente" : "Portal del presidente"} title={club.name} description="Gestiona tu plantilla y presenta la formación oficial de cada jornada." meta={<><span>Posición {String(standing?.position ?? 0).padStart(2, "0")}</span>{isDemoMode && <span>Modo de demostración</span>}</>}><div className="club-hero-crest" style={{ "--club-accent": club.color }}><ClubCrest club={club} size="xl" decorative /></div></PageHero>
      ...
      <section className="two-column-page">
        ...
        <div className="panel reveal-item"><SectionHeading eyebrow="Plantilla" title={`${players.length} jugadores activos`} />{players.length ? <div className="roster-list">{players.map((player) => <div key={player.id}><span>{player.shirtNumber ?? "—"}</span><strong>{player.name}</strong><small>{player.positionGroup === "GK" ? "Portero" : "Campo"}</small></div>)}</div> : <EmptyState title="Plantilla pendiente de cargar" description="Administración puede añadir jugadores desde su panel." />}</div>
      </section>
