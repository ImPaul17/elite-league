...
import { getPlayerPositions, getPlayerRosterGroup } from "../lib/playerPositions";
import { formatPlayerBirthDate, getAttributeTone, getPlayerFullName, getPlayerSummary, PLAYER_ATTRIBUTE_GROUPS } from "../lib/playerProfile";
...
export function PlayerPage({ player, club }) {
  ...
  const isGoalkeeper = getPlayerRosterGroup(player) === "GK";
  const facts = [
    ...
    ["Posiciones", getPlayerPositions(player).join(" · ") || "Sin posición"], ["Dorsal", player.shirtNumber ?? "—"],
  ];
  return <article className="player-profile-page">
    ...
    <div className="player-profile-intro">
      <header className="player-profile-hero">
        ...
        <PlayerCard player={player} club={club} headingAs="h1" />
      </header>
    ...
    <section className="player-summary-section" aria-labelledby="player-summary-title">
      <SectionHeading title="Atributos principales" eyebrow={isGoalkeeper ? "Portero" : "Jugador de campo"} id="player-summary-title" />
    ...
    <section className="player-attributes-section" aria-labelledby="player-attributes-title">
      ...
      <div className="player-attribute-grid">{PLAYER_ATTRIBUTE_GROUPS.map((group) => <section className={`player-attribute-group panel${group.id === "goalkeeping" && isGoalkeeper ? " is-primary" : ""}`} key={group.id} aria-labelledby={`player-attributes-${group.id}`}>
