...
import { normalizeUsername, validUsername, passwordError } from "../../supabase/functions/_shared/accountRules.js";
...
export function ClubAccountManager() {
  ...
  async function submit(event, action) {
    ...
    const validation = passwordError(selectedPassword) || (action === "create" && !validUsername(normalizeUsername(username)) ? "El usuario debe tener entre 3 y 32 caracteres: letras sin tildes, números, punto, guion o guion bajo." : "");
  ...
  return <div className="account-manager">
    <p>Crea un usuario por presidente y entrégale una contraseña temporal. Tendrá que cambiarla antes de utilizar su panel. Es preferible una contraseña temporal distinta para cada persona.</p>
    <form className="stack-form" onSubmit={(event) => submit(event, "create")} aria-busy={busy}>
      ...
      <label>Usuario<input value={username} onChange={(event) => setUsername(event.target.value)} minLength={3} maxLength={32} required disabled={busy} autoCapitalize="none" autoComplete="off" spellCheck={false} /></label>
      ...
      <label>Contraseña temporal<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={12} maxLength={128} required disabled={busy} autoComplete="new-password" /></label>
    ...
    <div className="account-manager-list">
      ...
      {accounts.map((account) => <div className="account-manager-row" key={`${account.userId}-${account.clubId}`}>
        ...
        {resetId === account.userId && <form className="stack-form account-reset" onSubmit={(event) => submit(event, "reset")}>
          ...
          <label>Nueva contraseña temporal<input type="password" value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} minLength={12} maxLength={128} required disabled={busy} autoComplete="new-password" /></label>
